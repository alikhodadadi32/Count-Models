# Negative Binomial Generalized Linear Model (NB-GLM) with JAGS
# refer to -- https://www.sciencedirect.com/science/article/pii/S0001457521001342 -- for more information about the model  

# Introduction:
# The Negative Binomial Generalized Linear Model (NB-GLM) is a statistical model used to analyze overdispersed count data, often encountered in ecological and epidemiological studies. 
...
# Model Specification:
# The NB-GLM model is specified using a text file named "NB-GLM.txt". The model specification includes the following components:
# Likelihood: The likelihood function for the Negative Binomial distribution.
# Priors: Prior distributions are specified for the model parameters, including the regression coefficients (beta_0, beta_1, beta_2, beta_3), the overdispersion parameter (phi).
# The model assumes a Negative Binomial distribution for the response variable y, with additional components to model overdispersion and incorporate covariates.
